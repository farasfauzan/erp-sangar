...
export default function OpnamePage() {
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Opname Pekerjaan (SPK)</h2>}
        >
            <Head title="Opname" />
            ...
            <div className="py-12">
                <div className="mx-auto max-w-7xl sm:px-6 lg:px-8 space-y-6">
                    ...
                    <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                        <div className="p-6">
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="text-lg font-bold">Catat Opname Progres Baru</h3>
                                <button
                            ...
                            {showForm && (
                                <form onSubmit={handleSubmit} className="space-y-4 mt-4 border-t pt-4">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700">Pilih SPK</label>
                                            <select
                                                required
                                                value={form.spk_id}
                                                onChange={e => { setForm({...form, spk_id: e.target.value}); }}
                                        ...
                                        <div>
                                            ...
                                            <input type="text" required value={form.opname_number} onChange={e => setForm({...form, opname_number: e.target.value})} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700">Tanggal Opname</label>
                                            <input type="date" required value={form.date} onChange={e => setForm({...form, date: e.target.value})} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700">Progres (%)</label>
                                            <input type="number" required min="0" max="100" step="0.01" value={form.progress_percentage} onChange={e => handlePercentageChange(e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm" />
                                        </div>
                                    </div>
                    ...
                    <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                        <div className="p-6">
                            <h3 className="text-lg font-bold mb-4">Riwayat Opname</h3>
                            {loading ? (
                                <p>Memuat data...</p>
                            ) : (
                                <table className="min-w-full divide-y divide-gray-200">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            ...
                                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">SPK</th>
                                            ...
                                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Nilai Klaim</th>
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {opnames.length === 0 ? (
                                            <tr><td colSpan="7" className="px-6 py-4 text-center text-sm text-gray-500">Belum ada data opname.</td></tr>
                                        ) : (
                                            opnames.map((opn, idx) => (
                                                <tr key={idx}>
                                                    <td className="px-6 py-4 text-sm font-medium text-gray-900">{opn.opname_number}</td>
                                                    <td className="px-6 py-4 text-sm text-gray-500">{opn.spk?.spk_number ?? '-'}</td>
                                                    <td className="px-6 py-4 text-sm text-gray-500">{opn.spk?.project?.project_name ?? '-'}</td>
                                                    <td className="px-6 py-4 text-sm text-gray-500">{opn.progress_percentage}%</td>
                                                    <td className="px-6 py-4 text-sm text-gray-500">Rp {Number(opn.amount).toLocaleString('id-ID')}</td>
                                                    <td className="px-6 py-4 text-sm">
                                                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${opn.status === 'APPROVED' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                                                            {opn.status}
                                                        </span>
                                                    </td>
                                                    <td className="px-6 py-4 text-sm text-gray-500">{opn.date}</td>
                                                </tr>
                                            ))
                                        )}
                                    </tbody>
