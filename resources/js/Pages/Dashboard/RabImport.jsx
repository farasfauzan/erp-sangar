...
function ExcelSheetGrid({ rows, rawData, sheet, savingRows, updateRow, saveRow }) {
    ...
    const columnWidths = useMemo(() => columns.map((_, columnIndex) => {
        const longest = previewRows.slice(0, 120).reduce((length, row) => {
            const value = String(Array.isArray(row.values) ? (row.values[columnIndex] ?? '') : (row.values?.[columns[columnIndex]] ?? ''));
            return Math.max(length, value.length);
        }, 0);
        return Math.max(90, Math.min(380, (longest * 7) + 24));
    }), [columns, rawData]);
    const categoryColumn = excelColumnName(columns.length + 1);
    const groupColumn = excelColumnName(columns.length + 2);
    const actionColumn = excelColumnName(columns.length + 3);
    const gridTemplateColumns = `64px ${columnWidths.map((width) => `${width}px`).join(' ')} 190px 230px 150px`;
    const totalWidth = 64 + columnWidths.reduce((total, width) => total + width, 0) + 570;
        ...
        <>
            ...
            <div ref={viewportRef} onScroll={(event) => setScrollTop(event.currentTarget.scrollTop)} className="h-[68vh] overflow-auto bg-white">
                <div style={{ minWidth: `${totalWidth}px` }}>
                    <div className="sticky top-0 z-30 grid h-7 bg-[#f1f3f4] text-center text-xs font-medium text-gray-600" style={{ gridTemplateColumns }}>
                        <div className="sticky left-0 z-40 flex items-center justify-center border-b border-r border-[#c7c7c7] bg-[#e8eaed]">#</div>
                        {columns.map((column) => <div key={column} className="flex items-center justify-center border-b border-r border-[#c7c7c7] bg-[#f1f3f4]">{column}</div>)}
                    ...
                    <div className="relative" style={{ height: `${previewRows.length * rowHeight}px` }}>
                        {visibleRows.map((rawRow, offset) => {
                            ...
                            return (
                                <div key={rawRow.row_number} className={`absolute left-0 grid text-[12px] text-gray-900 ${isSaved ? 'bg-[#edf7e8]' : row && !row.selected ? 'bg-gray-50 text-gray-400' : 'bg-white'} hover:bg-[#e8f0fe]`} style={{ top: `${(visibleStart + offset) * rowHeight}px`, height: `${rowHeight}px`, gridTemplateColumns, width: `${totalWidth}px` }}>
                                    <div className="sticky left-0 z-20 flex items-center justify-center border-b border-r border-[#d5d5d5] bg-[#f8f9fa] px-1 text-center font-normal text-gray-500">
                                    ...
                                    {columns.map((column, columnIndex) => {
                                        ...
                                        return <button key={column} type="button" title={value} onClick={() => setSelectedCell({ address, value })} className={`overflow-hidden text-ellipsis whitespace-nowrap border-b border-r border-[#d5d5d5] px-2 text-left ${!row && value ? 'font-medium' : ''}`}>{value}</button>;
                                    })}
                                    {row ? <>
                                        <div className="flex items-center border-b border-r border-[#d5d5d5] p-1">
                                            <select value={row.category} onChange={(event) => updateRow(row.id, 'category', event.target.value)} disabled={!row.selected || isConflict} className={`h-7 w-full rounded-full border px-2 text-[11px] font-medium focus:border-green-600 focus:ring-green-600 ${categoryStyle}`}>
                                                <option value="">Pilih kategori...</option>
                                                {MANUAL_CATEGORIES.map((category) => <option key={category.value} value={category.value}>{category.label}</option>)}
                                            </select>
                                        </div>
                                        <div className="flex items-center border-b border-r border-[#d5d5d5] p-1"><input value={row.group} onChange={(event) => updateRow(row.id, 'group', event.target.value)} disabled={!row.selected || isConflict} placeholder="Contoh: Struktur" className="h-7 w-full border-0 bg-transparent px-2 text-[11px] focus:bg-white focus:ring-2 focus:ring-green-600" /></div>
                                        <div className="flex items-center justify-center border-b border-r border-[#d5d5d5] p-1 text-center">
                                            <button type="button" disabled={!row.selected || !row.category || isSaving || isConflict} onClick={() => saveRow(row)} className={`min-w-[112px] rounded px-2 py-1 text-[10px] font-semibold transition disabled:cursor-not-allowed disabled:opacity-50 ${isSaved ? 'border border-green-300 bg-green-100 text-green-800' : isConflict ? 'border border-gray-300 bg-gray-100 text-gray-600' : 'bg-green-700 text-white hover:bg-green-800'}`}>
                                                {isSaving ? 'Menyimpan...' : isSaved ? '✓ Tersimpan' : isConflict ? 'Sudah diambil' : row.save_state === 'dirty' ? 'Masukkan ulang' : 'Masukkan'}
                                            </button>
                                        </div>
                                    </> : <>
                                        <div className="border-b border-r border-[#d5d5d5] bg-[#fafafa]" />
                                        <div className="border-b border-r border-[#d5d5d5] bg-[#fafafa]" />
                                        <div className="border-b border-r border-[#d5d5d5] bg-[#fafafa]" />
                                    </>}
                                </div>
                    ...
                    <div className="max-h-[68vh] overflow-auto bg-white">
                        <table className="min-w-[1540px] border-separate border-spacing-0 text-[12px] text-gray-900">
                            <thead className="sticky top-0 z-30">
                                ...
                                <tr className="h-10 bg-[#d9ead3] font-semibold text-[#1f3b24]">
                                    ...
                                    <th className="min-w-[110px] border-b border-r border-[#a8b9a1] px-2 text-left">Nomor / Kode</th>
                                    <th className="min-w-[380px] border-b border-r border-[#a8b9a1] px-2 text-left">Jenis Barang/Jasa</th>
                                    <th className="min-w-[90px] border-b border-r border-[#a8b9a1] px-2 text-center">Satuan</th>
                                    <th className="min-w-[100px] border-b border-r border-[#a8b9a1] px-2 text-right">Volume</th>
                                    <th className="min-w-[140px] border-b border-r border-[#a8b9a1] px-2 text-right">Harga Satuan</th>
                                    <th className="min-w-[155px] border-b border-r border-[#a8b9a1] px-2 text-right">Jumlah Harga</th>
                                    <th className="min-w-[190px] border-b border-r border-[#a8b9a1] px-2 text-left">Kategori</th>
                                    <th className="min-w-[230px] border-b border-r border-[#a8b9a1] px-2 text-left">Folder / Kelompok</th>
                                    <th className="min-w-[150px] border-b border-r border-[#a8b9a1] px-2 text-center">Masukkan</th>
                                </tr>
                            </thead>
                            <tbody>
                                {rows.map((row) => {
                                    ...
                                    return (
                                        <tr key={row.id} className={`${isSaved ? 'bg-[#edf7e8]' : row.selected ? 'bg-white' : 'bg-gray-50 opacity-60'} hover:bg-[#e8f0fe]`}>
                                            ...
                                            <td className="border-b border-r border-[#d5d5d5] px-2 py-1 font-mono text-gray-600">{row.code_item || '—'}</td>
                                            <td className="whitespace-normal break-words border-b border-r border-[#d5d5d5] px-2 py-1.5 font-medium leading-5" title={row.description}>{row.description}</td>
                                            <td className="border-b border-r border-[#d5d5d5] px-2 py-1 text-center">{row.unit || '—'}</td>
                                            <td className="border-b border-r border-[#d5d5d5] px-2 py-1 text-right font-mono">{Number(row.volume).toLocaleString('id-ID')}</td>
                                            <td className="border-b border-r border-[#d5d5d5] px-2 py-1 text-right font-mono">{Number(row.unit_price).toLocaleString('id-ID')}</td>
                                            <td className="border-b border-r border-[#d5d5d5] px-2 py-1 text-right font-mono font-semibold">{Number(row.total_price).toLocaleString('id-ID')}</td>
                                            <td className="border-b border-r border-[#d5d5d5] p-1">
                                                <select value={row.category} onChange={(e) => updateRow(row.id, 'category', e.target.value)} disabled={!row.selected || isConflict} className={`h-8 w-full rounded-full border px-2 text-[12px] font-medium focus:border-green-600 focus:ring-green-600 ${categoryStyle}`}>
                                                    <option value="">Pilih kategori...</option>
                                                    {MANUAL_CATEGORIES.map((cat) => <option key={cat.value} value={cat.value}>{cat.label}</option>)}
                                                </select>
                                            </td>
                                            <td className="border-b border-r border-[#d5d5d5] p-1"><input value={row.group} onChange={(e) => updateRow(row.id, 'group', e.target.value)} disabled={!row.selected || isConflict} placeholder="Contoh: Struktur" className="h-8 w-full border-0 bg-transparent px-2 text-[12px] focus:bg-white focus:ring-2 focus:ring-green-600" /></td>
                                            <td className="border-b border-r border-[#d5d5d5] p-1 text-center">
                                                <button type="button" disabled={!row.selected || !row.category || isSaving || isConflict} onClick={() => saveRow(row)} className={`min-w-[112px] rounded px-2 py-1.5 text-[11px] font-semibold transition disabled:cursor-not-allowed disabled:opacity-50 ${isSaved ? 'border border-green-300 bg-green-100 text-green-800' : isConflict ? 'border border-gray-300 bg-gray-100 text-gray-600' : 'bg-green-700 text-white hover:bg-green-800'}`}>
                                                    {isSaving ? 'Menyimpan...' : isSaved ? '✓ Tersimpan' : isConflict ? 'Sudah diambil' : row.save_state === 'dirty' ? 'Masukkan ulang' : 'Masukkan'}
                                                </button>
