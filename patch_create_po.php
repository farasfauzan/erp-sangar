...
file_put_contents($pagesDir . 'CreatePO.jsx', $createPoPageCode);
echo "Created CreatePO.jsx\n";

// Update routes/web.php
$webRoutesContent = file_get_contents($webRoutesFile);
if (!str_contains($webRoutesContent, "Route::get('/po/create'")) {
    $webRoutesContent .= "\nRoute::get('/po/create', function () {\n    return Inertia\\Inertia::render('CreatePO');\n})->middleware(['auth', 'verified'])->name('po.create');\n";
    file_put_contents($webRoutesFile, $webRoutesContent);
    echo "Updated routes/web.php\n";
}

// Ensure the '+ Buat PO Baru' button in PurchaseOrder.jsx links to /po/create
$poPage = file_get_contents($pagesDir . 'PurchaseOrder.jsx');
$poPage = str_replace(
...
$poPage = str_replace('</button>', '</a>', $poPage);
file_put_contents($pagesDir . 'PurchaseOrder.jsx', $poPage);
